...
abstract class ThemeContainer154 : ContainerControl
{
    ...
    void SetState(MouseState current)
    {
        State = current;
        Invalidate();
    }
    ...
    void CorrectBounds(Rectangle bounds)
    {
    ...
    bool smartBounds = true;
    public bool SmartBounds
    {
        get { return smartBounds; }
        set { smartBounds = value; }
    }
    ...
    public Bloom[] Colors
    {
        get
        {
            ...
            while (E.MoveNext()) T.Add(new Bloom(E.Current.Key, E.Current.Value));
    ...
    public string Customization
    {
        ...
        set
        {
            ...
            try
            {
                ...
                for (int I = 0; I <= Items.Length - 1; I++) Items[I].Value = Color.FromArgb(BitConverter.ToInt32(Data, I * 4));
            }
...
#pragma warning restore CA1303 // Literale nicht als lokalisierte Parameter übergeben
    ...
    Size imageSize;
    protected Size ImageSize => imageSize;

    bool isParentForm;
    protected bool IsParentForm => isParentForm;
    ...
    void InvalidateCustimization()
    {
        var M = new MemoryStream(Items.Count * 4);

        foreach (Bloom B in Colors) M.Write(BitConverter.GetBytes(B.Value.ToArgb()), 0, 4);
    ...
    Rectangle OffsetReturnRectangle;
    ...
    {
#pragma warning disable CA2002 // Auf Objekten mit schwacher Identität nicht sperren
        lock (MeasureGraphics) return MeasureGraphics.MeasureString(Text, Font, Width).ToSize();
#pragma warning restore CA2002 // Auf Objekten mit schwacher Identität nicht sperren
    }
...
#pragma warning restore CA2002 // Auf Objekten mit schwacher Identität nicht sperren
    ...
    LinearGradientBrush DrawGradientBrush;

    Rectangle DrawGradientRectangle;
    ...
    GraphicsPath DrawRadialPath;
    PathGradientBrush DrawRadialBrush1;
    LinearGradientBrush DrawRadialBrush2;

    Rectangle DrawRadialRectangle;
    ...
    GraphicsPath CreateRoundPath;

    Rectangle CreateRoundRectangle;
...
static class ThemeShare
{
    ...
    static List<AnimationDelegate> Callbacks = new List<AnimationDelegate>();
    static void HandleCallbacks(IntPtr state, bool reserve)
    {
        ...
        lock (Callbacks)
        {
            for (int I = 0; I <= Callbacks.Count - 1; I++) Callbacks[I].Invoke(Invalidate);
        }

        Frames += Rate;
    }

    static void InvalidateThemeTimer()
    {
...
#pragma warning restore CA1060 // Pinvokes in native Methodenklasse verschieben
{
    ...
    static extern bool CreateTimerQueueTimer(ref IntPtr handle, IntPtr queue, TimerDelegate callback, IntPtr state, uint dueTime, uint period, uint flags);
